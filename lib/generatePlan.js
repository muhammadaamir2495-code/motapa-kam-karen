...
const ROUTINE_LABEL = {
  ...
  night: "Night shift wala kaam",
  student: "Student",
  home: "Ghar sambhalne wali",
  senior: "Retired / senior citizen",
  athlete: "Sports / gym-heavy routine",
};
...
function proteinSources(dietPref) {
  const bank = {
    ...
    vegan: ["tofu", "dal", "chana", "sprouts", "soya chunks", "badaam ka doodh"],
    jain: ["paneer", "dal (bina pyaz-lehsun)", "chana", "sprouts", "soya chunks", "dahi"],
  };
  return bank[dietPref] || bank.veg;
}

function breakfastText(dietPref) {
  if (dietPref === "vegan") {
    return "1 katori oats + soya ya badaam ka doodh, mutthi bhar mewa";
  }
  if (dietPref === "veg" || dietPref === "jain") {
    return "1 katori oats + mutthi bhar mewa (badaam, akhrot)";
  }
  return "2 ubay anday ya 1 katori oats";
}
...
export function generatePlan(input) {
  ...
  if (goal === "muscletone") calorieTarget = tdee + 150;
  ...
  const timeline = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          text: `${proteins[0]} ya ${proteins[1]} + sabzi${dietPref === "jain" ? " (bina pyaz-lehsun, bina jad wali sabzi)" : ""}`,
          reason: "Protein - goal ke liye zaroori",
        },
  ...
  const goalNotes = {
    muscletone: [
      "Calories thori zyada rakhi gayi hain, protein zyada — muscle tone ke liye surplus zyada bara nahi hona chahiye.",
      "Sath halki strength training (bodyweight ya light weights) 3-4 din/hafta zaroori hai, sirf diet se tone nahi banega.",
    ],
    belly: [
  ...
  const skinNotes = {
    acne: [
      "Non-comedogenic (pores block na karne wale) moisturizer aur sunscreen use karen — moisturizer skip karna acne ko aur barhata hai.",
      "Salicylic acid ya benzoyl peroxide wala mild cleanser madad karta hai, lekin roz sirf ek active use karen, dono ek sath nahi.",
      "Pimples ko haath se na dabayen — isse scarring aur infection ka risk barh jata hai.",
    ],
